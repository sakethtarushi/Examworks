...
public class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
  ...
  public init(width: CGFloat, titles: [String]) {
    ...
    appearance = SlidingContainerSliderViewAppearance (
      backgroundColor: Constants.Constants.COLOR_APPTHEME,
  ...
  public func draw() {
    ...
    func orienationChangedUpdateFrames()
    {
        var labelTag = 0
        let width = CGFloat(self.frame.size.width) / CGFloat(self.titles.count)
        

        for i in 0...titles.count-1 {

            print("titles count \(i)")
        ...
        let selectorH = appearance.selectorHeight
        selector.frame = CGRect (x: 0, y: frame.size.height - selectorH, width: width, height: selectorH)
        selector.backgroundColor = appearance.selectorColor
