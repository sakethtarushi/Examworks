...
public class SlidingContainerViewController: UIViewController, UIScrollViewDelegate, SlidingContainerSliderViewDelegate {
  ...
  public init (parent: UIViewController, contentViewControllers: [UIViewController], titles: [String]) {
    ...
    sliderView.frame.origin.y = 64.0
    sliderView.autoresizingMask = [.flexibleWidth]
    ...
    for vc in contentViewControllers {
      vc.view.frame = CGRect (
        ...
        height: view.frame.size.height - 64 - parent.bottomLayoutGuide.length)
    ...
    override public func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: { (UIViewControllerTransitionCoordinatorContext) -> Void in
            ...
            self.sliderView.frame = CGRect(x: self.sliderView.frame.origin.x, y: self.sliderView.frame.origin.y, width: UIScreen.main.bounds.size.width, height: self.sliderView.frame.size.height)
            ...
            for vc in self.contentViewControllers {
                vc.view.frame = CGRect (
                    x: currentX,
                    y: 64,
                    width: self.contentScrollView.frame.size.width,
                    height: self.contentScrollView.frame.size.height - 64 )
                
                currentX += self.contentScrollView.frame.size.width
  ...
  public override func viewDidLoad() {
    ...
    func updateFramesOnOrientation() {
        ...
        setCurrentViewControllerAtIndex(presentShowingIndex)
      ...
      /*  var currentX: CGFloat = 0
        for vc in contentViewControllers {
            vc.view.frame = CGRect (
                ...
                y: 64,
                width: view.frame.size.width,
                height: view.frame.size.height - 64 - (self.parent?.bottomLayoutGuide.length)!)
